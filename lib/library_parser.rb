class LibraryParser
  ...
  def build_objects(artist_name, song_name, genre_name)
    song = Song.create(name: song_name.titleize)
    genre = Genre.find_or_create_by(name: genre_name.titleize)
    artist = Artist.find_or_create_by(name: artist_name.titleize)
